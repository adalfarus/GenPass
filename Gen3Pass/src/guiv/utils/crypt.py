...
def encrypt_data(data, key):
    """
    Encrypts data using AES encryption algorithm and a given key
    ...
    ciphertext = iv + cipher.encrypt(padded_data.encode('utf-8'))

    return ciphertext

def decrypt_data(ciphertext, key):
    """
    Decrypts data using AES encryption algorithm and a given key
    ...
    decrypted_data = cipher.decrypt(ciphertext[AES.block_size:]).rstrip(chr(AES.block_size).encode('utf-8'))
